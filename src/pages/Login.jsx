...
export default function Login() {
  ...
  const requestPhoneFromEitaa = () => {
    ...
    // }
    if (app) {
      // پیدا شد! حالا چک میکنیم تابع درخواست کانتکت را دارد؟
      if (app.requestContact) {
        app.requestContact((isShared, dataString) => {
          if (isShared) {
            alert("✅ شماره دریافت شد!");
            // دیتای خام را نشان بده تا بفهمیم چطوری پارس کنیم
            alert("Raw Data: " + JSON.stringify(dataString));
          } else {
            alert("❌ کاربر رد کرد.");
          }
        });
        ...
        alert(
          "شیء ایتا پیدا شد ولی تابع requestContact را ندارد! (شاید نیاز به آپدیت ایتا باشد)",
        );
        // برای اطمینان، محتویات اپ را نشان بده
        alert("App Keys: " + Object.keys(app));
      }
    } else {
      // 🚨 بخش دیباگ حیاتی:
      // اگر باز هم اینجا آمد، یعنی اسکریپت لود نشده یا اسمش چیز دیگری است.
      // ما کل متغیرهای window را می‌گردیم ببینیم چیزی شبیه eitaa هست؟
      const foundKeys = Object.keys(window).filter(
        (key) =>
          key.toLowerCase().includes("eitaa") ||
          key.toLowerCase().includes("web"),
      );

      alert("❌ شیء WebApp پیدا نشد.");
      alert("آیا این کلیدها کمک میکنند؟: " + foundKeys.join(", "));
    }
  };
